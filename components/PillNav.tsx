...
const PillNav: React.FC<PillNavProps> = ({
  ...
  const resolvedActiveColor = activeIndicatorColor ?? baseColor
  ...
  const cssVars = {
    ...
    ['--active-dot']: resolvedActiveColor,
  ...
  const allMobileItems = [...items, ...extraMobileLinks]

  return (
    <div className="relative z-[1000] w-full">
      <nav
        className={`w-full flex items-center justify-between md:justify-center box-border px-4 md:px-0 ${className}`}
        aria-label="Primary"
        style={cssVars}
      >
        {/* Logo */}
              ...
              return (
                <li key={item.href} role="none" className="flex h-full">
                  ...
                  >
                    ...
                    {isActive && (
                      <span
                        className="absolute left-1/2 -bottom-[6px] -translate-x-1/2 w-[6px] h-[6px] rounded-full z-[4]"
                        style={{ background: 'var(--active-dot, #eab308)' }}
                        aria-hidden="true"
                      />
                    )}
                  </Link>
          ...
          {allMobileItems.map((item) => {
            ...
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block py-3 px-4 text-[14px] font-semibold uppercase tracking-wide rounded-[50px] transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)]"
                  style={defaultStyle}
